...
export const getCategoriesFromTags = (tagsString: string): string[] => {
    ...
    const snippetTags = tagsString.toLowerCase().split(',').map(tag => tag.trim());
  ...
  export const determineCategories = (tagsString: string, detectedLang: string): CategoryInfo[] => {
    const snippetTags = tagsString.toLowerCase().split(',').map(tag => tag.trim());
    ...
    const hasLanguageTag = matchedCategories.some(category => category.kind === "language");
  
    if (!hasLanguageTag) {
      const languageCategory = categorizeLanguage(detectedLang);
      if (languageCategory !== 'unknown' && languageCategory in categories) {
        matchedCategories.push({ ...categories[languageCategory], autoDetected: true });
      }
    }
