...
export async function DELETE(request: Request) {
  ...
  if (!listID || !snippetID) {
    return NextResponse.json({ error: "listID and snippetID are required" }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ message: `Snippet ${snippetID} removed from list ${listID}` }, { status: 200 });
  } catch (error) {
