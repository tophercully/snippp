...
export async function PUT(request: Request) {
  ...
  if (!listID || !listName) {
    return NextResponse.json({ error: "listID and listName are required" }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ message: `List with ID ${listID} has been updated` }, { status: 200 });
  } catch (error) {
